...
$config["abs_url_images"] 		= "http://emptyjourneys.com/mobi/media/image/" ; 		//Absolute Images URL
$config["abs_url_videos"] 		= "http://emptyjourneys.com/mobi/media/media/" ; 		//Absolute Videos URL
$config["abs_url_audios"] 		= "http://emptyjourneys.com/mobi/media/media/" ; 		//Absolute Audio URL
$config["abs_url_files"] 		= "http://emptyjourneys.com/mobi/media/file/" ; 		//Absolute Files URL
...
switch($_GET["json"]){	
	...
	case "order":
		...
		// TODO: -+----+- SQL Query
		$sql = "SELECT * FROM `order` ".$where."ORDER BY ".$order_by." ".$sort_by." LIMIT 0, 200" ;
	...
	case "user":
		...
		// TODO: -+----+- SQL Query
		$sql = "SELECT * FROM `user` ".$where."ORDER BY ".$order_by." ".$sort_by." LIMIT 0, 200" ;
		if($result = $mysql->query($sql)){
			$z=0;
			while ($data = $result->fetch_array()){
				if(isset($data['id'])){$rest_api[$z]['id'] = $data['id'];}; # id
	...
	case "route":		$rest_api=array();
		$rest_api["site"]["name"] = "Empty Journeys" ;
		$rest_api["site"]["description"] = "Empty Journeys" ;
		$rest_api["site"]["imabuilder"] = "rev18.01.22" ;
