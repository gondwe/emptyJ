...
function reload($l=""){$h = $l == "" ? $_SERVER["REQUEST_URI"] : $l;$h = $l == "" ? "http://".$_SERVER["SERVER_NAME"].$h :$h;echo "<script>window.location.href='".$h."'</script>";}
    ...
    function profile_handler(){
        ...
        if(empty($complete = getlist("select * from `$truck_table` where user_id = '$id'"))){
			...
			$notify = checkrequest()? '<div class="notify"> <span class="heartbit"></span> <span class="point"></span> </div>' : null;
        switch($utype){
            case 1 : //owners
            echo ' <li><a href="./dashboard.php"><button class="btn btn-info btn-squared btn-xs">My Account</button></a></li>';
            ...
            echo '<li><a href="./truck_requests.php" ><button class="btn btn-danger btn-squared btn-xs">Truck Requests '.$notify.'</button></a></li>';
            break;
            case 2 : //drivers
            echo ' <li><a href="./dashboard.php"><button class="btn btn-info btn-squared btn-xs">My Account</button></a></li>';    
            ...
            break;
            case 3 : //customers
            echo ' <li><a href="./index.php"><button class="btn btn-danger btn-squared btn-xs">Homepage</button></a></li>';
            echo ' <li><a href="./dashboard.php"><button class="btn btn-info btn-squared btn-xs">My Account</button></a></li>';
            echo '<li><a href="./message.php" ><button class="btn btn-danger btn-squared btn-xs">Inbox</button></a></li>';
            echo '<li><a href="./truck_requests.php" ><button class="btn btn-danger btn-squared btn-xs">Truck Requests</button></a></li>';
            break;
            case 4: //admin    
            ...
            echo '<li><a href="./truck_requests.php" ><button class="btn btn-danger btn-squared btn-xs">Truck Requests</button></a></li>';
            echo '<li><a href="./truck_sms.php" ><button class="btn btn-success btn-squared btn-xs">Bulk Message</button></a></li>';
            break;
//            break;
            case 5 : //creators
            echo ' <li><a href="./dashboard.php"><button class="btn btn-info btn-squared btn-xs">My Account</button></a></li>';    
    ...
    function sub_status($me){
        ...
        $thismonth = date("m");
		$found = fetch("select status from truck_sub where user_id ='$me' and month(sub_month) = '$thismonth'");
		
        $lastmonth = count($found) > 1 ? date("m") - 1 : date("m");
		$prevdate  = fetch("select day(sub_month) from truck_sub where user_id ='$me' and month(sub_month) = '$lastmonth'");
		
		$day = $found == false ? date("d") : $prevdate;
		
		$sql = "select status from truck_sub where user_id ='$me' and month(sub_month) = '$thismonth' and day(sub_month) >= '$day'";
        return $sub_status = fetch($sql);
    }
	...
	$truck_types = getlist("select id, ucase(names) from truck_types");
    $truck_models = getlist("select id, ucase(names) from truck_models");