...
<a href="./addtruck.php" class="btn btn-info" >Add New</a>
</div>
</form>
...
// spill($trucks);
$users = getlist("select id, tel from truck_users");
...
if(!empty($trucks)) {
    ...
    echo "<tr><thead class='bg-info' style='color:#fff;'>"; foreach($ths as $th){ echo "<th>".rx($th)."</th>"; } echo "</thead></tr>";
    echo "<tbody>";
    $x = 1;
    foreach($trucks as $tds){
        ...
        $tds["driver_id"] = $users[$tds["driver_id"]];
        $tds["type"] = $truck_types[$tds["type"]];
